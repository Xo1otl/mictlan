...
# 認証したことによりadminユーザーであることが確定するため、以下のようにsessionではなくクエリパラメータを使って編集を行っても問題ない
authenticateAdmin();
        ...
        <?php elseif ($_GET['mode'] === 'r-option'): ?>
            ...
            <form action="update_r.php" method="post" id="r_options_form">
                ...
                <div>
                    ...
                    <input type="checkbox" id="ok" name="ok" <?php echo $profile->nsfwOptions->ok ? 'checked' : ''; ?>>
                </div>
                <div>
                    <label for="price">R作品</label>
                    <input type="number" id="price" name="price"
                        value="<?php echo htmlspecialchars($profile->nsfwOptions->price, ENT_QUOTES, 'UTF-8'); ?>"> 円/30分
                </div>
                <div>
                    <label for="hardOk">R過激表現オプション</label>
                    <input type="checkbox" id="hardOk" name="hardOk" <?php echo $profile->nsfwOptions->hardOk ? 'checked' : ''; ?>>
                </div>
                <div>
                    <label for="hardSurcharge">過激オプション割増</label>
                    <input type="number" id="hardSurcharge" name="hardSurcharge"
                        value="<?php echo htmlspecialchars($profile->nsfwOptions->hardSurcharge, ENT_QUOTES, 'UTF-8'); ?>">
                </div>
                <div>
            ...
            <form action="update_profile.php" method="post" id="profile_form">
                ...
                <div>
                    ...
                    <div class="radio-group">
                        ...
                        <label for="amateur">
                            <input type="radio" id="amateur" name="category" value="<?php echo \actor\Category::AMATEUR; ?>"
                                <?php echo \actor\Category::AMATEUR === $profile->category ? 'checked' : ''; ?>>
                            新人・アマチュア
                        </label>
                    </div>
                </div>

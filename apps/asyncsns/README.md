...
- 両方が存在する場合は太線で、リアルタイムコミュニケーションのための、コネクションは双方向矢印で表現しています
...
### (Auth)
- Authの役割は、ユーザー登録や認証です
- CognitoでOIDCやメール登録や認証を行います

### (Gateway)
- Gatewayの役割は、APIリクエストの統合的な制御です
- 例えば、jwtの一元的な検証、lambdaの統合、apiバージョンの切り替え、負荷対策などを行います
...
- コマンド処理apiでは、投稿、編集、リアルタイムチャット、通話などの処理を、Text Repository、Media Repository、RTC、Notificationなどのサービスと連携して実現します
    - 例えば投稿処理では、メディアファイルをアップロードし、テキストデータを保存して、友達に通知を送るといった一連の処理を行います
    - また、リアルタイムチャットでは、接続情報を管理し、メッセージをリアルタイムでやり取りし、オフラインのユーザーには通知を送るといった処理を行います
...
### (Media Repository)
- Media Repositoryの役割は、メディアファイルの永続化です
- 暗号化されたS3とTriggerを使った署名付きURLによる共有を利用することで、大容量メディアファイルをセキュアに保存・配信します
- また、MediaConvertを用いた動画データのストリーム形式への変換や保存、署名付きCookieによる共有を行います

### (Notification)
- Notificationの役割は、プッシュ通知やメールによる配信です
- SESやAmazon SNSで実装し、デッドレターキューで失敗時の分析や再処理も行います

### (CDN)
- CDNの役割は、コンテンツの効率的な配信とアクセス制御とグローバル展開です
- 例えば、証明書の一元管理や、Origin Access Controlポリシーに基づいたS3へのアクセス制御やキャッシュ配信やエッジ処理を、CloudFrontやWafやlambda@edgeで実装します

### (Observability)
- Observabilityの役割は、システム全体の監視と分析です
- CloudWatchによるパフォーマンスの収集・監視、X-Rayによる分散トレーシング、CloudTrailによるAPIやリソースの操作履歴の記録・監査、ManagedGrafanaによる統合的な可視化とダッシュボード作成を行います、またcloudfrontのリアルタイムログをopensearchで可視化します
...
- マネージドサービスをフル活用することで、インフラ運用の手間を減らし、従量課金でコストを最適化し、マネージドサービスによるマルチAZ構成などによって、高可用性とトラフィックに応じた自動スケーリングを実現します
- また、RTCとそのgatewayではroute53を用いてマルチリージョン構成にすることで、グローバル展開を目指します
...
- [signed cookie](https://docs.aws.amazon.com/ja_jp/AmazonCloudFront/latest/DeveloperGuide/private-content-choosing-signed-urls-cookies.html)
- [災害対策](https://aws.amazon.com/jp/blogs/news/disaster-recovery-dr-architecture-on-aws-part-iv-multi-site-active-active/)

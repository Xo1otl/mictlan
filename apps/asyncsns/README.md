...
- システム内の処理の流れは、副作用を持たないリードオンリーなクエリを破線、コマンドを実線で表現しています
...
- 例えば、jwtの一元的な検証やlambdaの呼び出しapiバージョンの切り替えや負荷対策などの機能を、API Gatewayで実装します
...
- コマンド処理apiでは、投稿、編集、リアルタイムチャット、通話などの処理を、Repository、Storage、RTC、Notificationなどのサービスと連携して実現します
    - 例えば、投稿処理では、ファイルをstorageに保存したり、テキストデータをrepositoryに保存したり、友達にNotificationを送ったりします
    - また、リアルタイムチャットでは、対象の接続がある場合にRTC経由でリアルタイムなチャット送信を行ったり、Notificationで通知を送ったりします
...
- AmazonSNSで実装し、デッドレターキューで失敗時の分析や再処理に対応します
